...
async function classifyAudio(apiKey, wavBase64, chartText, linkInfo) {
  const prompt = `You are a strict call-quality moderation AI for a social calling app. Listen to this audio call recording between a Host and a User.
...
- Match the category name EXACTLY as written in the chart.`;
  ...
  if (!resp.ok) {
    ...
    if (resp.status === 403) throw new Error('API key invalid/blocked hai — Settings mein check karo');
  ...
  // Category naam ko chart se fuzzy match karo (exactness ke liye)
  const chartCats = chartText.split('\n').map(l => l.split('|')[0].trim()).filter(Boolean);
  let cat = catM[1].trim().replace(/[*_#]/g, '');
  ...
  return {
    category: cat,
    confidence: (confM ? confM[1] : 'medium').toLowerCase(),
    reason: reaM ? reaM[1].trim() : ''
  };
}
...
async function startProcessing() {
  ...
  if (!apiKey) { toast('❌ Pehle Settings mein Gemini API key dalo!'); switchTab('page-settings'); return; }
  ...
  log(`🚀 Processing shuru: row ${s} se ${e} tak (${job.items.length} items)`, 'ok');
  processLoop(apiKey, chart);
}

async function processLoop(apiKey, chart) {
  const conc = parseInt((await kvGet('concurrency')) || '2');
  const pending = () => job.items.filter(i => i.status === 'pending');
  const workers = [];
  for (let w = 0; w < conc; w++) workers.push(worker(w + 1, apiKey, chart));
...
async function worker(wnum, apiKey, chart) {
  while (true) {
    // pause handling
    ...
    try {
      ...
      $('curItem').textContent = `🤖 Worker ${wnum}: Row ${item.row} — AI sun raha hai…`;
      const b64 = await blobToBase64(blob);
      let result = null, tries = 0;
      while (tries < 3) {
        try { result = await classifyAudio(apiKey, b64, chart, item); break; }
        catch (e) {
...
$('saveKey').addEventListener('click', async () => {
  ...
  await kvSet('gemini_key', k);
  $('keyStatus').innerHTML = '<span class="keyok">✅ Key save ho gayi</span>';
  toast('✅ API key saved');
});
$('testKey').addEventListener('click', async () => {
  const k = $('apiKey').value.trim() || (await kvGet('gemini_key')) || '';
  if (!k) { toast('❌ Pehle key dalo'); return; }
  $('keyStatus').innerHTML = '<span class="inf">🧪 Testing…</span>';
  try {
    const resp = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${encodeURIComponent(k)}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ contents: [{ parts: [{ text: 'Reply with exactly: OK' }] }], generationConfig: { maxOutputTokens: 5 } })
    });
    if (resp.ok) $('keyStatus').innerHTML = '<span class="keyok">✅ Key kaam kar rahi hai!</span>';
    else $('keyStatus').innerHTML = `<span class="keybad">❌ Key fail (${resp.status}) — dobara check karo</span>`;
  } catch (e) { $('keyStatus').innerHTML = '<span class="keybad">❌ Network error</span>'; }
});
...
(async function init() {
  await openDB();
  const k = await kvGet('gemini_key');
  if (k) { $('apiKey').value = k; $('keyStatus').innerHTML = '<span class="keyok">✅ Key saved hai</span>'; }
